...
public class Dangnhap_Activity extends Activity {
		...
		setContentView(R.layout.activity_dangnhap);
		 db=new database(getApplicationContext());
	      ...
	      //  Log.d("-----Size------",e+"");
	        ...
	        Log.d("-----Nam------",arrTaikhoan.get(0).getTenTK()+"");
	        Log.d("-----Nass------",arrTaikhoan.get(0).getMatKhau()+"");
	...
	public void dangNhap(){
		...
		if(name.equals(arrTaikhoan.get(0).getTenTK()+"") & pass.equals(arrTaikhoan.get(0).getMatKhau())){
			...
			for(int i=1;i<arrTaikhoan.size();i++){
				if(name.equals(name.equals(arrTaikhoan.get(i).getTenTK()) & pass.equals(arrTaikhoan.get(i).getMatKhau()))){
					Intent chuyen=new Intent(getApplicationContext(),BanCF_Activity.class);
					startActivity(chuyen);
				
					break;
				} else Toast.makeText(getApplicationContext(),"Tên đăng nhâp hoặc mật khẩu ko đúng", Toast.LENGTH_LONG).show();;
			}
		}
